...
/*   Updated: 2022/08/17 17:39:34 by mthiry           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    draw_map(t_data *data, int color, int height, int width)
{
    ...
    while (i != height)
    {
        ...
        while (j != width)
        {
            ...
            {
                data->pos_x = (j * (data->size_square)) + (data->size_square / 2) - (data->size_player / 2);
                data->pos_y = (i * (data->size_square)) + (data->size_square / 2) - (data->size_player / 2);
            }
            j++;
        }
        i++;
